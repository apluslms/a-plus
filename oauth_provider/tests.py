...
redirects her back to the Consumer's callback URL::
    ...
    >>> response.status_code
    200
    >>> 'oauth_token' in response.content
    True
    >>> 'printer.example.com' in response.content
    True
...
fixme: >>> response = c.get("/oauth/photo/", parameters)
fixme: >>> response.status_code
fixme: 401

fixme: >>> response.content
fixme: 'Invalid access token: ...'
...
class OAuthTestsBug10(TestCase):
    ...
    def setUp(self):
        ...
        resource = self.resource = Resource(name='testresource', url='/oauth/photo/')
        resource.save()
        ...
        self.request_token_parameters = {
            ...
            'scope': 'testresource',  # custom argument to specify Protected Resource
            }
